//
//  FinishCheckListEditViewController.swift
//  Agronom
//
...
final class FinishCheckListEditViewController: ListViewController {
    
    var viewModel: FinishCheckListEditViewModel?
...
extension FinishCheckListEditViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case is CheckListOdometrRow:
            ...
            cell.textField.text = "\(self.viewModel?.cdItem.odometer_end ?? 0)"
            
            cell.textChanged = { [weak self] text in
                guard let value = text?.int else { return }
                self?.viewModel?.cdItem.odometer_end = value.int16
            }
        ...
        case is CheckListLiquidRow:
            ...
            cell.textField.text = "\(self.viewModel?.cdItem.liquied_end ?? 0)"
            
            cell.textChanged = { [weak self] text in
                guard let value = text?.int else { return }
                self?.viewModel?.cdItem.liquied_end = value.int16
            }
            
            return cell
            
        case is CheckListGasRow:
            
            guard let cell = tableView.dequeueReusableCell(withIdentifier: "TaskParameterCell", for: indexPath) as? TaskParameterCell else {
                return UITableViewCell()
            }
            
            cell.titleLabel.text = "Газ"
            cell.textField.text = "\(self.viewModel?.cdItem.gas_end ?? 0)"
            
            cell.textChanged = { [weak self] text in
                guard let value = text?.int else { return }
                self?.viewModel?.cdItem.gas_end = value.int16
            }
